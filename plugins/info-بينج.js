 { generateWAMessageFromContent }  "@whiskeysockets/baileys";
import os from "os";
استيراد util from "util";
استيراد sizeFormatter from "human-readable";
import MessageType from "@whiskeysockets/baileys";
import fs from "fs";
استيراد { performance } from "perf_hooks";
...
const handler = async (m, { conn, usedPrefix }) => {
  ...
  ${tradutor.texto1[20]} ${modejadibot ? "مفعل" : "معطل"}`.trim();
  const doc = [
    "pdf",
    "zip",
  ...
  const Message = {
    ...
    contextInfo: {
      ...
      externalAdReply: {
        ...
        thumbnail: imagen1,
        sourceUrl: "instagram.com/gl_al.12",
      },
    },
    caption: info,
    footer: wm,
